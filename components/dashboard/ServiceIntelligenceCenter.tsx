...
const ServiceIntelligenceCenter: React.FC = () => {
  ...
  const serviceUsed = stats.monthlyServiceCount;
  const serviceLimit = EntitlementEngine.getLimit(tier, 'monthlyServiceLogs') as number;
  const canAddMore = serviceUsed < serviceLimit;
  ...
  const handleAddLog = () => {
    if (!canAddMore) {
      if (confirm(`Maintenance Quota Reached: Your current ${tier.toUpperCase()} plan allows ${serviceLimit} logs per month. Upgrade to Premium for unlimited logging?`)) {
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-2 no-print">
        <div className="space-y-3">
          ...
          <div className="flex gap-3 pt-6">
            {canExport ? (
              <>
                <button onClick={() => exportToCSV(serviceLogs, `Service_History_${activeVehicle?.model}`)} className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-6 py-3 rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] shadow-sm hover:bg-emerald-600 hover:text-white transition-all">📊 Excel</button>
        ...
        <button disabled={!activeVehicle} onClick={handleAddLog} className={`px-8 sm:px-12 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[10px] shadow-3xl transition-all flex items-center justify-center gap-3 ${canAddMore ? 'bg-slate-900 text-white hover:bg-blue-600' : 'bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed'}`}>
          <span className="text-xl">🛠️</span> {canAddMore ? 'Record Service' : 'Monthly Limit'}
        </button>
      </header>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 px-2">
            <div className="bg-white card-radius border border-slate-100 p-8 flex flex-col justify-between shadow-sm">
              <div className="space-y-4">
                ...
                <div className={`text-4xl font-black text-slate-900 tracking-tighter`}>{serviceUsed} <span className="text-xs text-slate-300 font-bold">/ {serviceLimit}</span></div>
              </div>
            </div>
          </div>
