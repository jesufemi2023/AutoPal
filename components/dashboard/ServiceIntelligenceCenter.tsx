      ...
      {activeTooltip === id && (
        ...
        >
          ...
          >
            ...
            <h4 className="text-[11px] font-black text-blue-500 uppercase tracking-[0.3em] mb-4">Service Intelligence</h4>
  ...
  return (
    <div className="space-y-12 sm:space-y-16 animate-slide-up pb-24 sm:pb-32">
      ...
      <div id="service-report-content" className="hidden">
        <div className="flex justify-between items-center border-b-4 border-blue-600 pb-8 mb-8">
          <div>
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-blue-600">Official Asset Maintenance Dossier</p>
          </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-y border-slate-200">
              ...
              <th className="p-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Service Type</th>
              <th className="p-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Provider</th>
              <th className="p-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Mileage</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {activeServiceLogs.map((log) => (
              <tr key={log.id}>
                <td className="p-4 text-[11px] font-bold text-slate-500">{formatDate(log.serviceDate)}</td>
                <td className="p-4 text-[12px] font-black text-slate-900 uppercase tracking-tight">{log.serviceType}</td>
                <td className="p-4 text-[10px] font-bold text-slate-400">{log.provider || 'Independent'}</td>
        ...
        <div className="mt-12 pt-8 border-t border-slate-100 flex justify-between items-end">
           <div className="space-y-4">
              <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">AutoPal Engineering Cloud // Generated: {new Date().toLocaleString()}</span>
           </div>
           <div className="text-right">
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Total Lifecycle Investment</p>
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-2">
        ...
        <button disabled={!activeVehicle} onClick={() => setShowLogTerminal(true)} className="bg-slate-900 text-white px-8 sm:px-12 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[10px] shadow-3xl hover:bg-blue-600 transition-all flex items-center justify-center gap-3 no-print">
      ...
      {!activeVehicle ? (
        ...
        <>
          ...
          <div className="space-y-8 px-2 no-print">
