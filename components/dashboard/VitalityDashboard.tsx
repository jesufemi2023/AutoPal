...
export const VitalityDashboard: React.FC<Props> = ({ vehicle, tasks, logs, fuelLogs }) => {
  ...
  // SOURCE OF TRUTH: AI AUDITED SCORE
  ...
  // DRIFT ENGINE: Check if logs have been added since the AI Judge last spoke
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6">
      {/* Neural Snapshot Banner (The Judge) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden border border-white/5 shadow-xl group">
          <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Neural Vitality</div>
          <div className="flex items-baseline gap-2">
            ...
            <div className="text-[9px] font-bold text-slate-500 uppercase">Audit</div>
          </div>
          {hasDrift && <div className="mt-4 text-[8px] font-black text-amber-500 uppercase tracking-widest animate-pulse flex items-center gap-2"><span>⚠️</span> Telemetry Drift Detected</div>}
        </div>

        <div className="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden border border-white/5 shadow-xl group">
          <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Neural Discipline</div>
          <div className="flex items-baseline gap-2">
      ...
      {/* Raw Telemetry Evidence (System Health) */}
      <div className="bg-white p-8 sm:p-10 rounded-[2rem] border border-slate-100 shadow-sm relative group flex flex-col transition-all duration-500 hover:shadow-lg w-full flex-grow">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
          <div className="space-y-1.5">
            <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.3em]">Health Telemetry</h4>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest opacity-80">Deterministic Baseline Data</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              <p className={`text-[10px] font-black uppercase tracking-widest leading-relaxed ${hasDrift ? 'text-amber-700' : 'text-slate-500'}`}>
                {hasDrift ? 'Audit Staleness Warning' : 'Status: Evidence Link Established'}
              </p>
              <p className="text-[8px] text-slate-400 font-black uppercase tracking-[0.2em]">
                {hasDrift ? 'New telemetry recorded since last scan. Score update recommended.' : `Last Verified: ${cachedAudit ? new Date(cachedAudit.timestamp).toLocaleDateString() : 'Never'}`}
              </p>
            </div>
          </div>
          {displayVitality === null && (
             <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Run Valuation to Audit</div>
          )}
        </div>
      </div>
    </div>
