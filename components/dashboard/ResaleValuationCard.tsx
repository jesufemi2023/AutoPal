...
export const ResaleValuationCard: React.FC<{
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Neural Audit Error. Please check your connection.");
    } finally {
    ...
    return (
      <div className="bg-slate-950 rounded-[2.5rem] p-12 text-white relative overflow-hidden shadow-3xl border border-blue-500/20 w-full h-full flex flex-col items-center justify-center min-h-[500px]">
        ...
        <div className="relative z-10 text-center space-y-8 max-w-sm">
          <div className="w-24 h-24 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto shadow-[0_0_40px_rgba(59,130,246,0.3)]"></div>
  ...
  return (
    <div className="w-full h-full space-y-6 flex flex-col">
        ...
        <div className="flex justify-between items-start relative z-10">
          <div className="space-y-6">
             ...
             <div className="space-y-1">
                ...
                <p className="text-[10px] text-blue-500/60 font-black uppercase tracking-widest">Confidence: {cachedReport.auditedScores.discipline}% (Verified Record Weight)</p>
             </div>
          </div>
      ...
      {/* Audit Detail Quadrants */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-grow">
        {/* Q1: Metabolic Audit Zone */}
        <div className="bg-white rounded-[2rem] p-8 border border-slate-100 space-y-6 shadow-sm">
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 p-4 rounded-2xl">
                 <div className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">True Efficiency</div>
                 <div className="text-xl font-black text-slate-900">{cachedReport.metabolicAudit.trueKml.toFixed(1)} <span className="text-[10px] font-bold">KM/L</span></div>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl">
                 <div className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Consumption Gap</div>
           ...
           <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-between">
              <div className="space-y-0.5">
                <span className="text-[9px] font-black text-rose-600 uppercase tracking-widest block">Monthly Neglect Tax</span>
                <span className="text-[7px] text-rose-400 font-bold uppercase tracking-widest">Est. waste due to inefficiency</span>
              </div>
              <span className="text-xl font-black text-rose-700">{formatCurrency(cachedReport.metabolicAudit.monthlyNeglectTax)}</span>
           </div>
        </div>

        {/* Q2: Diagnostics Zone */}
        <div className="bg-white rounded-[2rem] p-8 border border-slate-100 space-y-6 relative overflow-hidden shadow-sm">
           <div className="absolute top-0 right-0 p-4">
              <div className={`w-3 h-3 rounded-full ${cachedReport.diagnostics.severity === 'critical' ? 'bg-rose-500 animate-ping' : (cachedReport.diagnostics.severity === 'advisory' ? 'bg-amber-500' : 'bg-emerald-500')}`}></div>
           </div>
        ...
        {/* Q3: Recovery Parts Zone */}
        <div className="bg-slate-900 rounded-[2rem] p-8 text-white space-y-6 lg:col-span-2 shadow-xl">
           <div className="flex justify-between items-center">
              <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Recovery Strategy: Recommended Components</h4>
              <button onClick={handleAiAnalysis} className="text-[8px] font-black text-blue-500 uppercase tracking-widest">Recalibrate Scan</button>
           </div>
        ...
        {/* Q4: 5 Strategic Insights Zone */}
        <div className="bg-blue-600 rounded-[2rem] p-8 text-white space-y-6 lg:col-span-2 shadow-2xl shadow-blue-600/20">
           ...
           <div className="grid grid-cols-1 gap-4">
              {cachedReport.strategicInsights.map((insight, i) => (
                <div key={i} className="flex gap-4 items-start p-3 border-b border-blue-500/30 last:border-0 hover:bg-blue-700/50 rounded-xl transition-all">
                   <span className="text-xl opacity-40 shrink-0 font-black">0{i+1}</span>
      ...
      <div className="text-center pt-4 opacity-40">
         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Scan Timestamp: {new Date(cachedReport.timestamp).toLocaleString()} // Secure Audit ID: {cachedReport.timestamp.slice(-6)}</p>
      </div>
    </div>
  );
};
