...
const AssetIntelligenceCenter: React.FC<AssetIntelligenceCenterProps> = ({ mode }) => {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      // Secondary Phase: Optical Storage Sync
      if (imageFile && user?.id) {
        try {
          ...
          if (e.message?.includes('Permission Denied') || e.message?.includes('RLS')) {
            ...
            alert(`Optical Sync Failed: ${e.message || "Unknown error"}`);
          }
        }
      }
      ...
      if (errorMsg.includes('RLS') || errorMsg.includes('policy') || errorMsg.includes('PGRST116')) {
        errorMsg = `Security Protocol Violation: The cloud database rejected this asset update. This usually means the vehicle record belongs to another user or RLS policies are missing. Please consult SUPABASE_GUIDE.md.\n\nFull error: ${errorMsg}`;
      }
