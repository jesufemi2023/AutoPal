            ...
            {regStep === 'vin' ? (
              <form onSubmit={handleIdentifyAsset} className="space-y-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">17-Digit VIN</label>
                  <input type="text" required maxLength={17} placeholder="ABC1234567890XYZ" className="w-full px-8 py-6 bg-slate-50 border border-slate-100 rounded-[1.5rem] font-mono text-xl uppercase tracking-[0.1em] focus:ring-4 focus:ring-blue-600/5 outline-none text-center" value={newVin} onChange={(e) => setNewVin(e.target.value.toUpperCase())} />
                </div>
                <div className="space-y-4 pt-4">
                  <button disabled={isProcessing || newVin.length < 17} className="w-full bg-slate-900 text-white py-6 rounded-[1.5rem] font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-slate-900/20 disabled:opacity-20 transition-all">Identify Vehicle</button>
              ...
              <form onSubmit={handleFinalizeRegistration} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <select className="w-full px-6 py-4 bg-slate-50 rounded-2xl text-sm font-bold border border-slate-100 outline-none" value={manualData.bodyType} onChange={e => setManualData({...manualData, bodyType: e.target.value as BodyType})}>
