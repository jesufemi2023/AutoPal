...
const Dashboard: React.FC = () => {
  ...
  const handleIdentifyAsset = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setVinInfo("AI was unable to decode this VIN. Please enter the details manually.");
      // Small delay for UX so the user can read the message before the step transitions
      setTimeout(() => {
        setRegStep('manual');
      }, 2000);
    } finally {
  ...
  return (
    <div className="space-y-6 md:space-y-10 lg:space-y-16">
      ...
      {/* REGISTRATION MODAL: OPTIMIZED RESPONSIVE LAYOUT */}
      {showAddModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/90 backdrop-blur-3xl animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-5xl h-full md:h-[min(900px,95vh)] md:rounded-[3rem] shadow-4xl flex flex-col relative overflow-hidden border border-white/20">
            {isProcessing && (
            ...
            {/* STICKY HEADER - NEVER CHOPPED */}
            <div className="p-6 md:p-10 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 z-[100]">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
                      {regStep === 'vin' ? 'Stage 1: Asset ID Scanning' : 'Stage 2: Technical Calibration'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* SCROLLABLE CONTENT - DYNAMIC GRID */}
            <div className="flex-grow overflow-y-auto scrollbar-hide px-6 md:px-10 py-10 bg-slate-50/40">
              {regStep === 'vin' ? (
                <div className="max-w-xl mx-auto py-16 space-y-12">
                  <div className="text-center space-y-6">
                    <div className="w-24 h-24 bg-white rounded-[2.5rem] flex items-center justify-center text-4xl mx-auto shadow-2xl ring-1 ring-slate-100 animate-bounce-slow">🔍</div>
                ...
                <div className="space-y-10 pb-16">
                  ...
                  <div className="bg-white p-6 md:p-10 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-8">
                    ...
                    <div className="flex-1 text-center sm:text-left min-w-0">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-1">Authenticated Chassis ID</div>
                      <div className="text-2xl md:text-3xl font-mono font-bold text-slate-900 tracking-tighter break-all uppercase leading-none">{newVin || 'MANUAL_OVERRIDE_ACTIVE'}</div>
                      <div className="flex flex-wrap justify-center sm:justify-start gap-3 mt-5">
                        <span className="bg-blue-50 text-blue-600 text-[9px] font-black px-5 py-2 rounded-xl border border-blue-100 uppercase tracking-widest">Platform Sync Active</span>
                        <span className="bg-emerald-50 text-emerald-600 text-[9px] font-black px-5 py-2 rounded-xl border border-emerald-100 uppercase tracking-widest">Digital-Twin-Ready</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10">
                    ...
                    {/* SECTION 01: ORIGIN (FULL WIDTH) */}
                    <div className="lg:col-span-2 bg-white p-8 md:p-12 rounded-[3rem] border border-slate-200 shadow-sm space-y-10">
                      <div className="flex items-center gap-6 border-b border-slate-50 pb-8">
                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-xs shadow-lg shadow-blue-500/20">01</div>
                        <div>
                          <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.3em] leading-none">Manufacturer Credentials</h3>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-2">Required for factory roadmap mapping</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-3">
                          ...
                          <input type="text" required placeholder="e.g. G-Class" className="w-full px-8 py-5 bg-slate-50 border border-slate-100 rounded-2xl text-base font-bold focus:border-blue-500 focus:bg-white outline-none transition-all shadow-inner" value={manualData.model} onChange={e => setManualData({...manualData, model: e.target.value})} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-200 shadow-sm space-y-10">
                      <div className="flex items-center gap-6 border-b border-slate-50 pb-8">
                        ...
                        <div>
                          <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.3em] leading-none">Technical Blueprint</h3>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-2">Engineering performance data</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <input type="text" placeholder="e.g. 4.0L Bi-Turbo" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none" value={manualData.engineSize} onChange={e => setManualData({...manualData, engineSize: e.target.value})} />
                        </div>
                        <div className="col-span-2 space-y-3">
                          ...
                          <select className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none appearance-none" value={manualData.bodyType} onChange={e => setManualData({...manualData, bodyType: e.target.value as BodyType})}>
                            ...
                            <option value="truck">Truck / Pickup / Payload</option>
                            <option value="van">Van / MPV</option>
                            <option value="coupe">Coupe / Grand Tourer</option>
                          </select>
                        </div>
                        <div className="col-span-2 space-y-3">
                    ...
                    <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-200 shadow-sm space-y-10">
                      <div className="flex items-center gap-6 border-b border-slate-50 pb-8">
                        ...
                        <div>
                          <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.3em] leading-none">Lifecycle Baseline</h3>
                      ...
                      <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-3">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Tire Specification</label>
                            <input type="text" placeholder="e.g. 265/60 R18" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold focus:border-blue-500 outline-none" value={manualData.specs.tireSize} onChange={e => setManualData({...manualData, specs: {...manualData.specs, tireSize: e.target.value}})} />
                          </div>
                          <div className="space-y-3">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Oil Grade</label>
                            <input type="text" placeholder="e.g. 5W-40" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold focus:border-blue-500 outline-none" value={manualData.specs.oilGrade} onChange={e => setManualData({...manualData, specs: {...manualData.specs, oilGrade: e.target.value}})} />
                          </div>
                        </div>
                        <div className="space-y-3">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Battery Spec (Amps/Type)</label>
                          <input type="text" placeholder="e.g. 100Ah AGM" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold focus:border-blue-500 outline-none" value={manualData.specs.batteryType} onChange={e => setManualData({...manualData, specs: {...manualData.specs, batteryType: e.target.value}})} />
                        </div>
                      </div>
                    </div>
            ...
            <div className="p-6 md:p-10 bg-white border-t border-slate-100 flex flex-col sm:flex-row gap-4 md:gap-8 shrink-0 z-[100] shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.08)]">
              {regStep === 'vin' ? (
                <>
                  <button type="button" onClick={() => setRegStep('manual')} className="order-2 sm:order-1 flex-1 py-5 md:py-8 rounded-2xl md:rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-slate-900 transition-all">Manual Override</button>
                  <button disabled={newVin.length < 17} onClick={() => handleIdentifyAsset()} className="order-1 sm:order-2 flex-[2] bg-slate-900 text-white py-5 md:py-8 rounded-2xl md:rounded-[2.5rem] font-black uppercase text-[11px] tracking-[0.4em] shadow-2xl shadow-slate-900/30 hover:bg-blue-600 disabled:opacity-20 active:scale-[0.98] transition-all">Analyze Chassis ID</button>
                </>
              ) : (
                <>
