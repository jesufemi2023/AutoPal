...
const Marketplace: React.FC = () => {
  ...
  return (
    <div className="animate-slide-up space-y-8 md:space-y-12 lg:space-y-16">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 px-2">
        <div>
          <h2 className="text-4xl md:text-5xl lg:text-7xl font-black text-slate-900 tracking-tighter">Marketplace</h2>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-4">Verified Supply Chain Network v1.2</p>
        </div>
      ...
      {suggestedPartNames.length > 0 && (
        <div className="bg-slate-900 card-radius p-8 md:p-14 text-white relative overflow-hidden shadow-3xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
            <div className="w-16 h-16 bg-blue-600 rounded-[1.5rem] flex items-center justify-center text-2xl shadow-xl shadow-blue-500/20 shrink-0">✧</div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {[1,2,3,4].map(i => <div key={i} className="h-96 bg-white card-radius animate-pulse"></div>)}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
          {sortedItems.length > 0 ? sortedItems.map((item, idx) => {
            ...
            return (
              <div key={item.id} className={`bg-white card-radius p-8 border transition-all hover:shadow-2xl hover:translate-y-[-10px] flex flex-col group relative animate-slide-up`} style={{ animationDelay: `${idx * 0.05}s` }}>
                <div className="flex justify-between items-start mb-8">
                  <div className="flex flex-col gap-2">
                    ...
                    {isMatch && (
                       <span className="bg-blue-600 text-white text-[8px] font-black px-3 py-1.5 rounded-lg uppercase tracking-widest shadow-lg shadow-blue-500/20 self-start">
                        Engineering Match
                       </span>
                    )}
                  </div>
                  {item.isVerified && <div className="w-8 h-8 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center text-xs font-black">✓</div>}
                </div>
                
                <h3 className="text-2xl font-black text-slate-900 leading-tight mb-4 group-hover:text-blue-600 transition-colors">{item.name}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-10">
                  {item.compatibility.slice(0, 2).map(car => (
                    <span key={car} className="text-[9px] font-bold text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                      {car}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-auto pt-8 border-t border-slate-50 flex items-center justify-between">
                  <div>
                    <div className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1">Price</div>
                    <div className="text-2xl font-black text-slate-900 tracking-tighter leading-none">₦{item.price.toLocaleString()}</div>
                  </div>
                  <button 
                    onClick={() => {
                    ...
                    className="bg-slate-900 text-white w-12 h-12 rounded-xl flex items-center justify-center hover:bg-emerald-600 transition-all shadow-xl active:scale-90"
                  >
