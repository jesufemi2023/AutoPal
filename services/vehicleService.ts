...
const handleSupabaseError = (error: any, context: string) => {
  ...
  console.error(`Supabase Error [${context}]:`, { message, code: error?.code });

  if (error.code === '42501' || error.status === 403) {
    throw new Error(`Permission Denied: ${context} failed due to RLS policies. Check SUPABASE_GUIDE.md.`);
  }
  if (error.code === 'PGRST116') return null; 
  if (error.code === 'PGRST204' || error.code === '42703' || error.status === 400) return 'SCHEMA_MISMATCH';
  throw error;
};
...
export const updateVehicle = async (vehicleId: string, data: Partial<Vehicle>): Promise<Vehicle> => {
  ...
  if (error) {
    ...
    if (errType === 'SCHEMA_MISMATCH') {
       ...
       const retry = await supabase.from(DB_TABLES.VEHICLES).update(dbPayload).eq('id', vehicleId).select().single();
...
export const finalizeMaintenanceCompletion = async (
  ...
  const { data: logData, error: logError } = await supabase.from(DB_TABLES.RECORDS).insert([{
  ...
  const { data: ruleData, error: ruleError } = await supabase.from(DB_TABLES.RULES).update({
...
export const createManualServiceLog = async (vehicle: Vehicle, data: Omit<ServiceLog, 'id' | 'createdAt' | 'updatedAt'>): Promise<ServiceLog> => {
  return await createServiceLog(data);
};
